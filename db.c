...
void lock(locktype lt, pthread_rwlock_t *lk) {
    if (lt == l_read) {
...
node_t *node_constructor(char *arg_name, char *arg_value, node_t *arg_left,
    ...
    // initialize rwlock and error check
    int err;
    if ((err = (pthread_rwlock_init(&new_node->lock, 0))) != 0) {
        perror("pthread_rwlock_init, node constructor");
    }
...
int db_remove(char *name) {
    ...
    // if we found it, both parent and node are locked
    ...
    if (dnode->rchild == 0) {
        ...
        pthread_rwlock_unlock(&parent->lock);
        node_destructor(
            dnode);  // dont need to unlock dnode since its being destroyed
        ...
        pthread_rwlock_unlock(&parent->lock);
        next = dnode->rchild;
        lock(l_write, &next->lock);  // lock right child (only happens once)
        ...
        while (next->lchild != 0) {
            ...
            lock(l_write, &nextl->lock);  // lock left children
            pnext = &next->lchild;
...
node_t *search(char *name, node_t *parent, node_t **parentpp, locktype lt) {
    ...
    // PARENT SHOULD BE LOCKED UPON ENTRY
